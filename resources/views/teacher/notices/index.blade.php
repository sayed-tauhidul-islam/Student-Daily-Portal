<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-6xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            @else
                <form method="POST" action="{{ route('teacher.notices.store') }}" class="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
                    @csrf
                ...
                <form method="GET" class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
                    <div class="flex flex-col gap-3 md:flex-row md:items-center">
                        ...
                        <div class="flex gap-2">
                            ...
                            @if(!empty($search))
                                <a href="{{ route('teacher.notices.index') }}" class="rounded-2xl border border-slate-200 px-4 py-3.5 text-sm font-semibold text-slate-700">Reset</a>
                            @endif
                        </div>
                    </div>
                </form>
                    ...
                    @forelse($notices as $notice)
                        <article class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
                            <div class="flex flex-wrap items-start justify-between gap-3">
                                ...
                                <div class="flex gap-2">
                                    <a href="{{ route('teacher.notices.edit', $notice) }}" class="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700">Edit</a>
                                    <form method="POST" action="{{ route('teacher.notices.destroy', $notice) }}" data-confirm-delete>
                                        @csrf
                                        @method('DELETE')
