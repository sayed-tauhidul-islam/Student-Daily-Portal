<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('teacher.notices.update', $notice) }}" class="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
                @csrf
                @method('PUT')
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('teacher.notices.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3.5 text-sm font-semibold text-slate-700">Back</a>
                </div>
            </form>
        </div>
    </div>
